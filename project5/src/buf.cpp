...
int init_db(int num_buf)
{
    ...
    buf_mgr_latch = PTHREAD_MUTEX_INITIALIZER;
    return 0;
}
...
int buf_read_page(int table_id, pagenum_t pagenum)
{
    ...
    while (buffer_index != -1)
    {
        ...
        {
            LRU_func(buffer_index);
            buf[buffer_index].flag = true;
            pthread_mutex_lock(&buf[buffer_index].page_latch);
            pthread_mutex_unlock(&buf_mgr_latch);
            return buffer_index;
        }
